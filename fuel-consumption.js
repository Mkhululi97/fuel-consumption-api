...
export default function FuelConsumptionAPI(db) {
  async function addVehicle({ description, regNumber }) {
    ...
    const result = await db.oneOrNone(
      `insert into fuel_api.vehicles (description, reg_number) values ($1, $2) returning id`,
  ...
  async function vehicles() {
    return await db.manyOrNone(`select * from fuel_api.vehicles`);
  }

  async function vehicle(id) {
    return await db.oneOrNone(`select * from fuel_api.vehicles where id = $1`, [
      id,
    ]);
  }
  ...
  async function refuel(vehicleId, liters, amount, distance, filled_up) {
    ...
    const result = await db.oneOrNone(
      `
            insert into fuel_api.fuel_entries(
              vehicle_id,
                liters,
                amount_paid,
                distance,
        ...
        `,
      [vehicleId, liters, amount, distance, filled_up]
    );
    ...
    const results = await db.oneOrNone(
      `
            ...
            from fuel_api.fuel_entries 
            where vehicle_id = $1`,
      [vehicleId]
    );

    await db.none(
      `update fuel_api.vehicles set 
            total_amount = $1, 
    ...
    const lastTwoEntriesForCar = await db.manyOrNone(
      `
            select * from fuel_api.fuel_entries 
            where vehicle_id = $1
    ...
    await db.none(
      `update fuel_api.vehicles set fuel_consumption = $1 where id = $2`,
